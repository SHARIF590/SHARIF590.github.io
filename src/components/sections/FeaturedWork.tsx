...
export function FeaturedWork() {
  return (
    <section id="work" className="py-24 sm:py-32 border-t border-white/10 bg-[#0A0A0A]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-mono tracking-wider uppercase text-neutral-300">
              <span
                className="h-1.5 w-1.5 rounded-full"
                style={{ backgroundColor: "var(--accent-color, #10B981)" }}
              />
              <span>Skiper 3D Interactive Deck</span>
            </div>
            <h2 className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-white font-syne">
              Featured <span style={{ color: "var(--accent-color, #10B981)" }}>Creations</span>
            </h2>
            <p className="mt-3 text-base text-neutral-400 leading-relaxed max-w-2xl font-poppins">
              Hover across each project card to experience dynamic 3D perspective tilt, real-time light sheen reflections, and production architectures.
            </p>
          </div>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 text-xs font-mono font-medium text-neutral-300 hover:text-white transition-colors self-start sm:self-auto"
          >
        ...
        {/* Project Cards with Skiper 3D Tilt */}
        <div className="mt-14 flex flex-col gap-10 sm:gap-14">
          {projects.map((project) => (
            <Skiper3DCard
              ...
              maxTilt={7}
              glareOpacity={0.18}
              className="bg-[#111111] hover:border-white/25"
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">

                {/* Visual Preview Container */}
                <div className="lg:col-span-6 bg-[#0B0B0B] p-6 sm:p-8 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-white/10">
                  {/* Top status bar */}
                  ...
                  {/* Project Image */}
                  <div className="relative my-6 rounded-xl border border-white/10 bg-[#0A0A0A] overflow-hidden aspect-[16/10] group">
                    <img
                      src={project.image}
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-4xl font-bold font-syne text-white/15">
                      {project.number}
                    </span>
                ...
                <div className="lg:col-span-6 p-6 sm:p-8 flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl sm:text-3xl font-bold text-white tracking-tight font-syne">
                      {project.title}
                    </h3>
                  ...
                  <div className="mt-8 pt-6 border-t border-white/10">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {project.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-lg bg-white/[0.04] px-3 py-1 text-xs font-mono text-neutral-300 border border-white/10"
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center gap-4">
                      <a
                        ...
                        className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-xs font-semibold text-black hover:bg-neutral-200 transition-all duration-200 hover:scale-105"
                      >
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/[0.03] px-5 py-2.5 text-xs font-semibold text-neutral-300 hover:text-white hover:border-white/30 transition-all duration-200"
                      >
